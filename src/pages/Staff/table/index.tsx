...
import { Typography, Chip, Drawer, SvgIcon, Tab } from "@mui/material";
...
import { UpdateStatus, resendInvite } from "services/staff";
...
const StaffTable = ({ value, searchText, drawer, setDrawer }: any) => {
  ...
  const {
    ...
    isFetching,
  }: any = useGetStaff({ query: { status: value, ...query } });
  ...
  const options = (rowData: any) => [
    {
      ...
      content: () => {
        // only allow recruiter status for non-awaiting
  ...
  const changeRecruiterStatus = () => {
    ...
    UpdateStatus({
      ...
      body: {
        isActive: recruiterStatus?.status == "active" ? "inactive" : "active",
      },
    })
      ?.then((res) => {
        refetchStaff();
        ...
        <Box sx={style}>
          ...
          >
            ...
            <Stack sx={{ textAlign: "center" }}>
              <Typography variant="h4">{`${recruiterStatus?.status == "active" ? "Deactivate" : "Activate"} Staff`}</Typography>
